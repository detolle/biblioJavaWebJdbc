...
<form id="form1" action="<%=request.getContextPath()%>/emprunt" method="post">
...
<label for="idu">Identifiant Utilisateur</label>
<input type="text" class="form-control" name="idu" id="idu" value="" placeholder="" required/>
</fieldset>
</c:if>
	...
	<caption></caption>
		...
		<tbody>
			<tr>
				...
				<td><fmt:formatDate pattern="dd/MM/yyyy" value="${utilisateur.dateNaissance}"/></td>
...
<c:if test="${not empty idExemplaire}">
	...
	<!--div id="message" class="bg-primary text-white">${message}</div-->
</c:if>
...
<input type="button" class="btn btn-dark text-white" onClick="document.location.href='<%=request.getContextPath()%>/emprunt'" value="Réinitialiser" />
<c:if test="${not empty idExemplaire}">
